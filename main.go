...
func NewApp() (*App, error) {
	...
	fiberApp.Use(cors.New(cors.Config{
		AllowOrigins:     "http://localhost:3000, http://localhost:3001, https://www.treato.org", // Allow all origins
	...
	fiberApp.Use(func(c *fiber.Ctx) error {
		c.Set("Content-Security-Policy",
			"default-src 'self'; "+
				"connect-src 'self' http://localhost:3000 http://localhost:3001  http://localhost:8080; "+
	...
	if err != nil {
		...
		webhooksGroup.Post("/workos", func(c *fiber.Ctx) error {
			...
			a.Logger.Debug("received WorkOS webhook request",
				...
				zap.Int("content_length", c.Context().Request.Header.ContentLength()))

			// IMPORTANT: DO NOT read the body here with c.Body()
			// Just pass the context to the handler
